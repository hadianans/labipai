...
export default function Index({ auth, announcements }) {
    ...
    return (
        ...
        >
            ...
            <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg p-4 sm:p-6 xl:mx-8">
                ...
                <div className="overflow-x-auto rounded-md">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                <th scope="col" className="px-3 py-2 sm:px-4 sm:py-3 text-left text-xs lg:text-sm font-medium text-gray-500 uppercase tracking-wider w-16">Image</th>
                                <th scope="col" className="px-3 py-2 sm:px-4 sm:py-3 text-left text-xs lg:text-sm font-medium text-gray-500 uppercase tracking-wider">Title</th>
                                <th scope="col" className="hidden lg:table-cell px-4 py-2 text-left text-xs lg:text-sm font-medium text-gray-500 uppercase tracking-wider w-24">Priority</th>
                                <th scope="col" className="hidden md:table-cell px-4 py-2 text-left text-xs lg:text-sm font-medium text-gray-500 uppercase tracking-wider w-24">Status</th>
                                <th scope="col" className="hidden xl:table-cell px-4 py-2 text-left text-xs lg:text-sm font-medium text-gray-500 uppercase tracking-wider w-40">Duration</th>
                                <th scope="col" className="hidden sm:table-cell px-4 py-2 text-right text-xs lg:text-sm font-medium text-gray-500 uppercase tracking-wider w-32">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr className={expandedRows.includes(item.id) ? 'bg-gray-50' : ''}>
                                        <td className="px-3 py-2 sm:px-4 sm:py-3 whitespace-nowrap">
                                            <div className="flex items-center gap-2 sm:gap-3">
                                                ...
                                                {item.img_url ? (
                                                    <img className="h-8 w-8 sm:h-10 sm:w-10 rounded object-cover" src={`/storage/${item.img_url}`} alt="" />
                                                ) : (
                                                    <div className="h-8 w-8 sm:h-12 sm:w-12 rounded bg-gray-200 flex items-center justify-center text-gray-500 text-[8px] sm:text-xs">No Img</div>
                                                )}
                                            </div>
                                        </td>
                                        <td className="px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm font-medium text-gray-900 truncate max-w-[clamp(120px,40vw,300px)] sm:max-w-[clamp(150px,30vw,360px)]" title={item.title}>{item.title}</td>
                                        <td className="hidden lg:table-cell px-4 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-500">
                            ...
                            {announcements.length === 0 && (
                                <tr>
                                    <td colSpan="6" className="px-6 py-4 text-center text-sm text-gray-500">No announcements found.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
